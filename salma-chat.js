...
function salmaRenderRoute(routeData) {
  ...
  var stopsHTML = pois.map(function(stop, idx) {
    ...
    var mapsUrl = stop.lat && stop.lng ? 'https://www.google.com/maps?q=' + stop.lat + ',' + stop.lng : 'https://www.google.com/maps/search/' + encodeURIComponent(stop.name || stop.headline || 'Parada');
  ...
  // Mapa Leaflet
  if (hasMapData && typeof L !== 'undefined') {
    setTimeout(function() {
      ...
      if (coords.length > 1) L.polyline(coords, { color: '#d4a017', weight: 2.5, opacity: .8, dashArray: '8,6' }).addTo(routeMap);
      ...
      if (coords.length > 1) { routeMap.fitBounds(coords, { padding: [40, 40] }); } else { routeMap.setView(coords[0], 12); }
    }, 200);
  }
...
async function salmaGuardarRuta() {
  try {
    ...
    var raw = window._salmaLastRoute;
    var routeToSave = {
      nombre: raw.title || raw.titulo || 'Mi ruta',
      titulo: raw.title || raw.titulo || 'Mi ruta',
      destino: raw.country || raw.destino || '',
      dias: raw.duration_days || raw.dias || 0,
      desc: raw.summary || raw.descripcion || '',
      summary: raw.summary || '',
      budget_level: raw.budget_level || '',
      tags: Array.isArray(raw.tags) ? raw.tags : [],
      tips: Array.isArray(raw.tips) ? raw.tips : [],
      suggestions: Array.isArray(raw.suggestions) ? raw.suggestions : [],
      raw_route: raw,
      pois: Array.isArray(raw.stops)
        ? raw.stops.map(function (s, i) {
            return {
              id: s.id || ('stop_' + (i + 1)),
              ...
              mapsUrl: s.lat && s.lng
                ...
                : ('https://www.google.com/maps/search/' + encodeURIComponent(s.name || s.headline || 'Parada'))
            };
          })
        : []
    };
