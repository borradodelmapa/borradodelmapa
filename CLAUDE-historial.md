# CLAUDE-historial.md — Borrado del Mapa (archivo histórico)

Recortado de `CLAUDE.md` el 25 sept 2026, a petición de Paco ("RECORTA CLAUDE.MD") —
`CLAUDE.md` se relee entero en cada turno de cada sesión, y con más de 4800 líneas era
buena parte de por qué trabajar aquí salía caro. Este archivo guarda el historial
completo de sesiones, bugs ya resueltos e investigaciones — no se relee automáticamente,
solo se abre si hace falta reconstruir "por qué se hizo así" o "qué se probó ya".

**`CLAUDE.md` sigue siendo el único sitio para pendientes activos y protocolo.** Si algo
de aquí vuelve a fallar, no lo trates como "ya resuelto" solo porque está aquí — repórtalo
como pendiente nuevo en `CLAUDE.md`.
...
- **PANEL ADMIN — PESTAÑAS QUE FALTAN (pedido por Paco el 23 sept 2026, al cerrar el día):**
  ...
  **HECHO 23 sept 2026 (tarde) — INGRESOS: `GET /admin/revenue` (Worker `bfd15c65-62cb-4f40-af05-5c764d5aaec7`, commit `2cf30bf8`, comprobado contra `/version`, 21 secretos intactos) + pestaña Ingresos del panel `2026-09-23.8` (commit `327128b`), SIN CONFIRMAR con la sesión real de Paco:** lista las Checkout Sessions pagadas de Stripe con la MISMA `STRIPE_SECRET_KEY` que ya usa el Worker (no hace falta otra clave; el modo test/real sale del prefijo `sk_test_`/`sk_live_` y el panel avisa con un banner). Tarjetas: este mes, 30 días, total, y margen estimado del mes (ingresos/1,21 − gasto Google estimado − Claude estimado a 0,92 €/USD; sin comisiones de Stripe ni reembolsos ni otros proveedores). Por plan y últimas 20 compras. Probado con Stripe simulado y en jsdom (0 errores JS). **Aviso de coste §8:** las lecturas de Stripe no se facturan; sin coste. **HECHO 23 sept 2026 (noche) — panel `2026-09-23.9` (insignia de versión grande: 16 px, borde naranja; pedido por Paco porque no la veía) y `2026-09-23.10` (commit `a885fed`): "Resumen del mes" arriba del Dashboard** con 4 tarjetas — Ingresos del mes (Stripe, con etiqueta PRUEBA si la clave es sk_test_), Gastos estimados (Google + Claude), Margen estimado (ingresos sin IVA − gastos) y Usuarios (total · Premium · guías), más una nota que dice qué falta si un dato falla; las tarjetas de ingresos, gastos y usuarios llevan a su pestaña. Cada dato se pide por separado (`/admin/revenue`, `/admin/google-usage`, `/admin/stats`): si uno falla, su hueco queda en "—". Solo panel, sin cambios en el Worker ni coste. SIN CONFIRMAR con la sesión real de Paco. **Ojo, sin resolver:** Paco dijo "no sale" de la pestaña Ingresos con el panel ya en `.8` (la insignia lo confirmaba, así que NO era caché); en pruebas simuladas funciona (escritorio y menú móvil). Falta saber qué ve exactamente: si el menú no la tiene, si sale vacía, o si sale un error rojo; con eso, `wrangler tail` mientras pulsa. **HECHO 23 sept 2026 (noche) — punto de salud REAL de Anthropic:** `/health` ahora llama también a `/v1/messages/count_tokens` (por el Gateway de Cloudflare, con `claude-sonnet-4-6`; **endpoint gratuito**, no genera texto ni se factura; comprobado que responde 200) — Worker `85dc6f0b-4837-4de2-baa6-c4621c86de2d`, commit `3f1385a2`; panel `2026-09-23.11` con el punto "Anthropic" en el Dashboard (antes el punto de Claude era en realidad el de OpenAI). Aviso §8: sin coste. La comprobación guardada por `/health` (10 min) no lleva Anthropic hasta la próxima real ("Comprobar ahora" en Configuración). **Sigue pendiente de esta lista:** Usuarios con gestión (Paco: "hay que pensarlo"), Analítica (GA4_CREDENTIALS), BigQuery real — **Worker LISTO Y DESPLEGADO (`GET /admin/google-real`, Worker `a04b7c2a-89da-4463-a3f1-c35d183ee370`, commit `a0c4857b`, 21 secretos intactos; devuelve coste real de hoy/ayer/mes/por SKU neto de créditos, cacheado 30 min en KV `gbq:cache`, `?force=1` lo salta; probado 16/16 con BigQuery simulado), pero FALTA QUE PACO DÉ PERMISOS y luego el lado del panel.** Pasos de Paco: (1) copiar el email de la cuenta de servicio del Worker (Firebase Console → Configuración del proyecto → Cuentas de servicio → `firebase-adminsdk-…@borradodelmapa-85257.iam.gserviceaccount.com`); (2) Google Cloud, proyecto `Salma Project` (`gen-lang-client-0108818247`) → IAM y administración → IAM → Conceder acceso → pegar ese email y darle los roles **BigQuery Job User** y **BigQuery Data Viewer**; (3) comprobar en BigQuery que el dataset `billing_export` ya tiene la tabla `gcp_billing_export_v1_012460_9B02AE_D84C54` (Google la crea al llegar el primer dato, unas horas tras activar la exportación). Códigos de error del endpoint: `no_permission` (403, falta el paso 2), `no_table` (falta el 3), `timeout`, `bq_error`. Aviso §8: la consulta lee unos MB de una tabla particionada (BigQuery da 1 TB/mes gratis): sin coste real. **HECHO 23 sept 2026 (noche): Paco dio los permisos** (`firebase-adminsdk-fbsvc@borradodelmapa-85257.iam.gserviceaccount.com` con `Usuario de trabajo de BigQuery` + `Visualizador de datos de BigQuery` en Salma Project; Google avisó de que tarda unos minutos) **y el panel `2026-09-23.12` (commit `51d7b76`) ya pinta el coste real:** sección "Coste REAL según Google" en Gastos (hoy incompleto, ayer, mes con créditos, por servicio y últimos días; "Actualizar" fuerza consulta nueva) y el Resumen del Dashboard y el margen de Ingresos usan el real si está disponible (etiquetado REAL) y, si no, la estimación. Avisos claros si falta permiso (`no_permission`) o la tabla aún no existe (`no_table`). Probado en jsdom con los 3 casos, 0 errores JS. **SIN CONFIRMAR con datos reales: el Worker todavía no se ha visto responder con la consulta de verdad (necesita sesión de admin); el primer resultado real lo verá Paco en Gastos.** **ANALÍTICA (23 sept 2026, noche) — EN CURSO, Paco haciendo los pasos uno a uno:** en vez de crear un secret nuevo, `/ga4` del Worker (`78097445-a4e0-446a-8111-6061a1a0a0ed`, commit `ff8f3d5a`) usa la MISMA cuenta de servicio de Firebase (`firebase-adminsdk-fbsvc@borradodelmapa-85257.iam.gserviceaccount.com`, scope `analytics.readonly`) si no hay `GA4_CREDENTIALS`; panel `2026-09-23.13` con errores legibles. Pasos de Paco: (1) activar "Google Analytics Data API" en el proyecto `borradodelmapa-85257` (https://console.cloud.google.com/apis/library/analyticsdata.googleapis.com?project=borradodelmapa-85257); (2) en Google Analytics, propiedad 352732094 → Administrar acceso → añadir ese email como **Lector**. Sin coste (la Data API es gratuita). **Pasos 1 y 2 HECHOS por Paco (captura): la API ya estaba habilitada en `borradodelmapa-85257` y la cuenta `firebase-adminsdk-fbsvc@…` ya es Lector de la propiedad "ANALISIS WEB › PACO" (352732094).** Detalle: esa propiedad ya tenía otro Lector, `admin-analytics@gen-lang-client-0108818247.iam.gserviceaccount.com` (cuenta creada antes en Salma Project; el Worker no la usa). **CONFIRMADO EN PANTALLA por Paco (23 sept 2026, noche): la pestaña Analytics del panel muestra datos reales (30 días: 236 sesiones, 22 usuarios, 20 nuevos, gráfico de visitas por día) usando la cuenta de Firebase, sin `GA4_CREDENTIALS`. Ese secret YA NO HACE FALTA. CERRADO.** Panel `2026-09-23.14` (commit siguiente a `596dc15`): la tarjeta "Visitas hoy" del Dashboard (antes siempre "—") ahora sale de GA4 (hoy y "ayer N"), se refresca en cada visita al Dashboard. **Panel `2026-09-23.15`: bloque "Analítica · últimos 7 días" en el Dashboard (pedido por Paco: "analítica visión rápida en dashboard")** — sesiones, usuarios, nuevos y duración media, sesiones de hoy/ayer, minigráfico por día, y top 3 países y páginas; pulsar el bloque lleva a la pestaña Analytics; si Analytics falla lo dice y el resto del Dashboard sigue. Cada informe va por separado (4 llamadas a `/ga4`, gratuitas). Probado en jsdom (0 errores JS); SIN CONFIRMAR con la sesión real de Paco. **Panel `2026-09-23.16`: orden del Dashboard pedido por Paco — primero Resumen del mes (ingresos, gastos, margen, usuarios) y Gasto en Google, y DESPUÉS la Analítica. Panel `2026-09-23.17`: ENLACES DIRECTOS a las páginas de cada proveedor** ("directo es directo"): una fila de enlaces bajo cada bloque del Dashboard y arriba de cada pestaña (Ingresos→Stripe, Gastos→facturación/cuotas/presupuesto de Google, BigQuery, Anthropic, OpenAI; Analytics→Google Analytics; Usuarios→Firebase) y la lista completa de 20 en Configuración (4 grupos, incl. Cloudflare, GitHub para desplegar el Worker, Twilio, Duffel, RapidAPI, ElevenLabs, Serper, Brave, Resend). Todos salen de UN sitio: `LINKS`/`LINK_GROUPS` en `config.js` del panel (ahí se cambian si cambia un proyecto). Los de Stripe cambian solos de prueba a real según el modo de la clave. Probado en jsdom (40 enlaces, sin errores JS) y comprobado que las direcciones responden; SIN CONFIRMAR con la sesión real de Paco. **FEEDBACK DE TESTERS EN EL PANEL (24 sept; orden de Paco: costes reales (7) y feedback (5) primero, embudo (4) luego, historial y coste por guía (2, 3) cuando haya datos de testers; alertas por email DESCARTADAS porque el panel es lo primero que mira):** Worker `975dc5ee-a6c6-45c4-b7e6-e4550ac0d4a5` (commit `20d7760f`): `GET /admin/feedback` (100 últimos de `beta_feedback`, logs recortados a 6.000 car.) y `POST /admin/feedback-seen`. Panel `2026-09-24.5`: pestaña 💬 Feedback con el número de sin ver en la pestaña, filtro Sin ver/Todos, captura, versiones, logs desplegables, "Copiar todo" (para pegar en Claude Code) y marcar visto/no visto. Probado (9 en Worker, 12 en jsdom); SIN CONFIRMAR con sesión real. Sigue: costes reales de Anthropic y OpenAI (necesita que Paco cree una clave de administrador en cada consola, paso a paso con URL directa). **Panel `2026-09-24.4` (24 sept, pedido por Paco: "veo usuarios, tengo que pulsar uno a uno para verlo, mejor directo en la misma tabla"): la tabla de Usuarios muestra TODO sin abrir la ficha** — usuario (nombre+correo), registro, último acceso, plan (etiqueta Premium/Gratis), Premium hasta, guías, mensajes/guías/cambios del mes, coste Claude del mes y con qué entra, todo ordenable (números como números), y **botones de acción en la propia fila** (+30 d de Premium, Quitar, Bloquear/Habilitar, Ficha), con el mismo aviso de confirmación; el resultado sale bajo el buscador. La ficha completa sigue disponible con "Ficha" o pulsando el resto de la fila. Probado 25+ comprobaciones en jsdom (0 errores JS); SIN CONFIRMAR con la sesión real de Paco. **ACTUALIZACIÓN 24 sept 2026: la tabla `gcp_billing_export_v1_012460_9B02AE_D84C54` YA TIENE DATOS (captura de Paco, pestaña Detalles: 10 filas, 2 particiones, creada 23 sept 22:55 y última modificación 23 sept 23:27 hora de España), pero el coste real sale 0,00 € porque esas líneas parecen estar a 0 € (dentro de lo gratuito o sin tarifar).** Para no ocultarlo, Worker `fea730d0-bec1-48d2-be6f-3ed0a13802ba` (commit `532451e2`) devuelve `rows_read`, `zero_skus` y `first_day`, y el panel `2026-09-24.3` dice "Google ya ha registrado N líneas de uso, todas a 0,00 €" en vez de "Sin gasto". También panel `2026-09-24.2`: las barras de reparto por servicio ya no salen en rojo. Volver a mirar Gastos en 1-2 días: cuando Google tarifique los usos de Places aparecerán importes. **CONFIRMADO 23 sept 2026 (noche, captura de Paco): la consulta a BigQuery FUNCIONA de punta a punta (sin error de permiso ni de tabla, así que el permiso está activo y la tabla existe), pero devuelve 0,00 € en hoy/ayer/mes y "Sin gasto este mes": la exportación todavía no tiene filas (se activó hace pocas horas; Google tarda hasta ~24 h en escribir el primer dato y hasta ~5 días en rellenar el mes anterior).** Volver a mirar Gastos mañana 24 sept: si sigue a 0 pasadas ~24-48 h de la activación, revisar en BigQuery que la tabla `gcp_billing_export_v1_012460_9B02AE_D84C54` tenga filas y que la exportación siga en "Costo de uso estándar" habilitada., y lo de **Cars (RapidAPI, decisión de Paco)**.
**HECHO 24 sept 2026 — USUARIOS CON GESTIÓN (Paco: "4 usuarios vamos hacerlo"):** Worker `7f50c900-508e-4908-8d43-8369f9eab229` (commit `b38b5b65`, 21 secretos intactos): `POST /admin/user-action` con 5 acciones — `premium_add` (1-730 días, se suman a lo vigente; escribe `premium_until`+`isPremium`+`premium_last_plan: manual-admin` con la cuenta de servicio), `premium_remove`, `disable`/`enable` (Firebase Authentication vía Identity Toolkit, scope cloud-platform; al deshabilitar revoca sesiones con `validSince`) y `reset_free` (borra KV `usage:{uid}:total`). **Nunca borra cuentas ni datos.** Cada acción queda en KV `adminlog:{ms}:{uid}` (1 año) y en los logs (`[ADMIN-ACCION]`). `/admin/stats` ahora trae también `disabled`, `last_login` y `providers` desde Authentication (`auth_ok` false si falla, sin romper). Panel `2026-09-24.1` (commit siguiente a `7c45cce`): tabla con columna Último acceso y etiqueta roja DESHABILITADA; al pulsar una fila se abre la ficha (correo, registro, último acceso, proveedor, estado, plan, guías, uso del mes, enlace a su ficha en Firebase) con los botones de acción, cada uno con aviso de confirmación que dice qué pasará y a quién. Probado 21/21 en el Worker (con Firestore y Authentication simulados) y 25 comprobaciones en jsdom (una encontró un fallo real —la ficha no se abría— ya corregido). **SIN CONFIRMAR con la sesión real de Paco, y sin probar aún contra Firebase Authentication real** (el permiso de la cuenta `firebase-adminsdk-fbsvc` es "Administrador de Firebase Authentication", debería bastar). Aviso §8: sin coste (Auth y Firestore, cuotas gratuitas).
...
**24 sept 2026 — COSTE REAL DE OPENAI EN EL PANEL: DESPLEGADO, SIN CONFIRMAR con la sesión real de Paco.** Worker `8c4c8f7f-427e-469a-b107-9b009b1262a7` (commit `81fab712`): `GET /admin/openai-costs` (solo admin) lee `https://api.openai.com/v1/organization/costs` del mes con el secret nuevo `OPENAI_ADMIN_KEY` (clave de administrador de OpenAI, solo lectura de costes; puesta por Paco; 22 secrets ahora). Devuelve mes/hoy en USD, por concepto y por día; caché KV `oai:cache` 30 min (`?force=1`). Panel `2026-09-24.6` (Gastos → "Coste REAL de OpenAI"). **Aviso §8:** leer costes no se factura; no añade ninguna llamada de pago. **Anthropic real: NO posible hoy** — la cuenta de Paco es "Paco's Individual Org" y la API de costes de Anthropic exige organización Team ("Convertir en Team" en Settings → Organization; sin probar qué cambia). Dato manual de su consola (24 sept): 87,10 US$ este mes, casi todo la clave "salma api vietnam" (86,62 US$; el nombre es solo una etiqueta del 16 abril), caché con 16 % de aciertos. Mientras tanto vale la estimación por usuario del Worker.

**24 sept 2026 — MODAL "RUTA COMPLETA" (`map-modal.js`) YA NO PREGUNTA A GOOGLE POR CADA PARADA: DESPLEGADO, SIN CONFIRMAR EN PANTALLA.** Antes, cada apertura pedía al navegador un `PlacesService.getDetails` por parada (fuera del Worker, sin caché ni tope propio). Ahora `_resolvePoint` pide primero `GET /place-details` al Worker, que devuelve `lat`/`lng` del catálogo `pl:` (0 llamadas a Google si el lugar ya está; 1 llamada una sola vez si no) y solo cae al `getDetails` del navegador si eso falla. Commits `b4fefd07` y `9d98904f`, `map-modal.js?v=11`, **Worker Version ID `5b088ac6-3f21-4686-b0d9-dbb2da2f86b4`** (confirmado contra `/version`). `/place-details` ahora devuelve `lat`/`lng` y renueva UNA vez las fichas v1 del catálogo (antiguas, sin coordenadas; probado: `miss` → `hit` con coordenadas). **Aviso §8:** baja las llamadas del navegador; el coste de renovar las fichas v1 es puntual (≤ ~74 fichas × céntimos, una sola vez, solo cuando alguien las abre). **Sigue abierto en este modal:** la línea de la ruta usa `DirectionsService` del navegador (1 llamada por apertura, sin caché) y el buscador interno usa Places SearchBox; `Geocoder` de respaldo. Siguiente paso posible: dibujar la línea con `/directions` del Worker (ya cacheado, permanente).
**CONFIRMADO EN PANTALLA por Paco (24 sept 2026):** el modal "Ruta completa" abre bien (paradas numeradas y línea de ruta) con el cambio anterior. El contador del Worker no subió al abrirlo dos veces (solo un Find Place suelto, +0,01 €). El ahorro real en llamadas del navegador no es visible en el panel: se comprobará en Google Cloud → métricas de Places API o en la factura por SKU.

**PENDIENTES DEL PANEL ADMIN Y COSTES — resumen al cierre del 24 sept 2026 (Paco se va a pasear; estado: todo lo de arriba desplegado, Worker `5b088ac6`, panel `2026-09-24.7`).**
1. **Google real (BigQuery):** mirar Gastos → "Actualizar" en 1-2 días; si sigue a 0 pasadas 48 h desde el 23 sept ~23:00, revisar filas de la tabla `gcp_billing_export_v1_012460_9B02AE_D84C54`. Comparar entonces con la factura de Google por SKU (Informes) y con Cloud Console → métricas de Places API, para confirmar que las guías y el modal "Ruta completa" ya no gastan.
2. **Anthropic real:** imposible con "Paco's Individual Org"; exige "Convertir en Team" (Settings → Organization). Paco decide; antes leer el texto de confirmación. Mientras tanto, dato manual de la consola (24 sept: 87,10 US$ del mes, clave "salma api vietnam" 86,62 US$, caché 16 %) y estimación del Worker.
3. **OpenAI real:** HECHO y confirmado (0,9 US$ del mes; secret `OPENAI_ADMIN_KEY`).
4. **Modal "Ruta completa":** siguiente paso opcional = dibujar la línea con `/directions` del Worker (caché permanente) en vez de `DirectionsService` del navegador (1 llamada por apertura); el buscador interno del modal usa Places SearchBox y hay un `Geocoder` de respaldo. Pedir OK a Paco antes (§8).
5. **Más adelante (orden de Paco):** embudo (visitas → registros → primera guía → Premium); historial de gastos y coste real por usuario/guía cuando haya datos de testers.
