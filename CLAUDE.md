...
- **Ideas sacadas de una app similar (Kabi, 22 sept 2026) — SOLO ESTUDIO, SIN IMPLEMENTAR NADA.** Paco vio una app parecida (basada en guías ya hechas por otros viajeros/redactores, no generadas por IA como las nuestras — esa sigue siendo nuestra ventaja de fondo) y mandó capturas de varias pantallas suyas. Quedan aquí solo como catálogo para revisar con calma, nada de esto se ha tocado en código:
  1. **Bottom bar con botón "+" central (FAB)** — su barra es Inicio/Explorar/**+**/Viajes/Perfil, con un botón grande de crear en el centro, más visible que un tab normal. Nuestro equivalente hoy es el tab "Chat" (ahí es donde se genera una guía), pero no tiene ese protagonismo visual. Candidato a rediseño de la bottom bar (hoy: Ayuda/Chat/Rutas/Perfil) — cambio de maquetación, no de lógica.
     **DECIDIDO por Paco (22 sept 2026): SE HACE, pero más tarde — no es de las 6 ideas aparcadas sin más, tiene ya diseño acordado, solo falta que él diga "ahora".** Es pura navegación, sin crear ninguna pantalla nueva: el creador de ruta (destino+días, "Afinar" con los otros 6 campos, botón "Trazar ruta →") **ya existe** — es la propia pantalla del tab "Salma" cuando no hay ruta activa (`_ceBilleteHTML`, [app.js:361](app.js:361)), y el botón "Trazar nueva ruta +" ya la reabre sin tocar la ruta guardada cuando sí hay una activa ([app.js:551](app.js:551)). El "+" del bottom bar solo tiene que llevar a esa misma pantalla desde cualquier sitio de la app. Único matiz decidido: si hay una **conversación sin guardar** en curso (chateando con Salma, sin haber pulsado aún "Crear ruta con mapa" — una ruta ya GUARDADA nunca corre este riesgo, vive en Firestore aparte) y se pulsa el "+", **avisa antes de descartarla** ("¿Empezar una ruta nueva? Perderás la conversación actual", Cancelar/Continuar) — no la borra en silencio como hoy hace "Trazar nueva ruta +" (que ahí no hace falta el aviso porque no hay nada sin guardar que perder). Enlazado con el pendiente "Ruta por X sin días" de más abajo — el "+" es la vía fiable que sustituye a intentar cazar la intención por texto libre.
