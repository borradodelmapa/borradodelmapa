...
- **Ideas sacadas de una app similar (Kabi, 22 sept 2026) — SOLO ESTUDIO, SIN IMPLEMENTAR NADA.** Paco vio una app parecida (basada en guías ya hechas por otros viajeros/redactores, no generadas por IA como las nuestras — esa sigue siendo nuestra ventaja de fondo) y mandó capturas de varias pantallas suyas. Quedan aquí solo como catálogo para revisar con calma, nada de esto se ha tocado en código:
  ...
  4. **Emojis en los botones/chips** — retoque visual pendiente. Paco añadió el 22 sept: que sean naranjas, en TODOS los chips. **Aviso técnico, sin implementar todavía**: un emoji Unicode de verdad (📍, 🔍...) es un glifo a todo color que el navegador dibuja siempre igual — `color` en CSS NO lo recolorea (a diferencia de un icono SVG con `stroke="currentColor"`, que sí, y es lo que ya usa `.chip-icon` en algún sitio). Para tenerlos en naranja de verdad hace falta ir por iconos SVG monocromo en vez de emoji Unicode — pendiente de decidir con Paco si quiere eso (más curro, un SVG por chip) o prefiere emoji reales tal cual salen (sin poder forzar el color). Tampoco se ha decidido qué emoji/icono va en cada chip.
